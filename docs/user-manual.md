...
#### `/distill` — 会話から候補を抽出して Proposal 化

`/bye yes` より少し賢い抽出です。ユーザー発話のキーワードで type を振り分けます。

```text
/distill              # decision + lesson + preference
/distill decision
/distill lesson
/distill preference
```

| ヒント例（発話に含むと拾いやすい） | type |
|----------------------------------|------|
| decide, 決定, 方針, 採用, will use… | decision |
| lesson, 学び, 失敗, avoid, 壊れた… | lesson |
| prefer, 好み, always, デフォルト… | preference |

- 最大 8 件、source session 付き
- ヒットゼロで type を 1 種に限定しているときだけ、直近 3 ユーザー発話を fallback
- 正本化はしない → `/inbox` → `/apply` / `/reject`
...
| `/bye yes` または `/bye -y` | **`/distill all` と同じエンジン**（キーワード抽出 + ヒットなし時は直近ユーザー発話を decision に fallback） |
| 会話が空 | 何も書かず終了メッセージ |

`/bye yes` と `/distill` は共通実装です。セッション中は `/distill`、終了確認付きなら `/bye` → `/bye yes`。タイトルを自分で付けるなら **`/capture`**。
...
| `/skim owner/repo [1d\|3d\|…]` | 任意 repo の直近変更を per-commit diff 付きで skim → `GH:…/skim/日付` に追記 |
| `/grok <質問>` / `/grok <x.com URL> <質問>` | **Grok CLI** に時事・X 投稿の説明を委任（本体モデルは DeepSeek 等のまま） |
...
| `logseq_set_planning` | 既存タスクの DEADLINE / SCHEDULED を設定・クリア |
| `logseq_upsert_task` | タイトル一致でタスクを作成または更新（冪等） |
...
| GH news が dashboard に出ない | ファイル名が `%3A` / `%2F` エンコードでも検出される。`/sync-gh` 後に `[[GitHub]]` を確認 |

## 8. クイック smoke（手元確認）

設定の `logseq_graph` が指すグラフで、壊さない範囲の確認手順です。

1. **構造化タスク**  
   会話: 「今日の journal に TODO Buy milk を #Task で追加して」→ Logseq で `* TODO Buy milk #Task` を確認。
2. **set / upsert**  
   「Buy milk を DONE に」→ `logseq_set_todo` / upsert 経路。planning は「期限を 2026-08-01 に」。
3. **Record 学習**  
   `/capture decision Smoke test | ok` → `/inbox` → `/apply <id>` → `Records/decision/…` と `nyanclaw/audit`。
4. **distill**  
   決定っぽい一文を会話に出したあと `/distill` → pending が増える。不要なら `/reject`。
5. **GitHub（任意）**  
   `/sync-gh` → `GH:…/news` と `[[GitHub]]` dashboard。encode 済みファイル名でも recent が載る。
6. **Repo skim**  
   `/skim phpstan/phpstan-src 1d` → `[[GH:phpstan/phpstan-src/skim/YYYY-MM-DD]]` に run セクション追記（最大 20 コミット）。local path は `GH:owner/repo` の `:clone_path:`、探索ルートは config `repo_explore_roots`。

自動テスト: `bun test`（org / session / records / gh-news / skim）。

### Grok 委任（時事・X）

主エージェントが DeepSeek などでも、**公開 Web / X の鮮度**が要るときは Grok CLI に任せます。

```text
/grok 今日の AI 関連の話題を3つ
/grok https://x.com/Yorimen/status/1037851101209362432 このツイートを説明して
```

自然言語でも可（エージェントが `ask_grok` を呼ぶ）:

> このツイートについて説明して https://x.com/…

```yaml
# config.yaml（任意）
grok_model: grok-4.5          # 既定
# grok_bin: /Users/you/.grok/bin/grok
```

前提: [Grok CLI](https://x.ai/cli) を入れ、`grok login` 済み。

### Repo skim と clone_path

```yaml
# config.yaml（探索ルートのみ — repo ごとの map ではない）
repo_explore_roots:
  - ~/src
  - ~/work
```
...
```org
# pages の GH:phpstan/phpstan-src.org など
* phpstan/phpstan-src
:PROPERTIES:
:clone_path: ~/src/phpstan-src
:END:
```

探索で path が見つかっても、`:clone_path:` への自動書き込みはしません（確認後に手で書く想定）。
